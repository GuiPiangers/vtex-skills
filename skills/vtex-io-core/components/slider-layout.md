...
![{"base64":"  ","img":{"width":2632,"height":1332,"type":"png","mime":"image/png","wUnits":"px","hUnits":"px","length":1341359,"url":"https://cdn.jsdelivr.net/gh/vtexdocs/dev-portal-content@main/images/vtex-slider-layout-0.png"}}](https://cdn.jsdelivr.net/gh/vtexdocs/dev-portal-content@main/images/vtex-slider-layout-0.png)
...
Slider Layout enables you to integrate your component with [Google Analytics 4](<https://developers.google.com/analytics/devguides/collection/ga4>). The integration happens through the [`GA4 view_promotion`](<https://developers.google.com/analytics/devguides/collection/ga4/reference/events?client_type=gtm#view_promotion>) event, which is typically associated with the promotion banners carousel displayed by the Slider Layout.
...
![{"base64":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAACCAIAAADwyuo0AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAJUlEQVR4nAEaAOX/ANfV1Kypo6Gemff29wCempsYEgodFw3a2tzORw4N2wgzoAAAAABJRU5ErkJggg==","img":{"src":"https://vtexhelp.vtexassets.com/assets/docs/src/gtm-site-editor___bc52365aafad63deb5bfed1d74f307c0.png","width":819,"height":464,"type":"png"}}](https://vtexhelp.vtexassets.com/assets/docs/src/gtm-site-editor___bc52365aafad63deb5bfed1d74f307c0.png)
...
---|---  
`slider-layout`| ![{"base64":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAABCAIAAAB2XpiaAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAFUlEQVR4nGOYJCn0v6/sS1lCLQMDACRKBQjZFNj+AAAAAElFTkSuQmCC","img":{"src":"https://img.shields.io/badge/-Mandatory-red","width":69,"height":20,"type":"svg"}}](https://img.shields.io/badge/-Mandatory-red) Builds block sliders for your store through its `children` list blocks.  
...
### Product Shelf Carousel (Desktop arrows only)
...
### Hero Banners (Autoplay + stop on hover)
...
```json
{
  "slider-layout#gallery": {
    ...
    "children": ["image#1", "image#2", "image#3", "image#4"]
  }
}
```

## Slider Layout Group (Hard Rules)

When using `slider-layout-group` to sync multiple sliders:

- Every `slider-layout` inside the group must have the same `props` and values.
- The only allowed difference between them is the `children` list.
- If you need different behaviors, use separate groups (do not mix configs in one group).

## Performance Notes

- Keep `itemsPerPage` conservative (especially on desktop) to avoid heavy first paint.
- Prefer image lazy loading for banner-heavy sliders.
- Avoid very short autoplay timeouts (increases layout work and hurts UX).

## Common Pitfalls

- Missing responsive values inside `itemsPerPage` leads to inconsistent behavior across breakpoints.
- Using pagination dots everywhere on dense shelves increases visual noise; default to `never` for shelves.
